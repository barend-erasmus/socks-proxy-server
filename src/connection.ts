...
export class Connection {
  ...
  protected handleAuthenticationRequest(data: Buffer): void {
    ...
    const authenticationStatus: AuthenticationStatus = this.authenticateFn(password, userName)
  ...
  protected async handleConnectionRequest(data: Buffer): Promise<void> {
    ...
    if (addressType === AddressType.DOMAIN_NAME) {
      ...
      this.sendConnectionResponse(
        addressType,
        ConnectionStatus.ADDRESS_TYPE_NOT_SUPPORTED,
        ipAddressBytes,
        portBytes,
        version,
      );
    ...
    switch (commandCode) {
      case CommandCode.TCPIP_PORT_CONNECTION:
        this.sendConnectionResponse(
          addressType,
          ConnectionStatus.COMMAND_NOT_SUPPORTED,
          ipAddressBytes,
          portBytes,
          version,
        );
  ...
  protected handleGreetingRequest(data: Buffer): void {
    ...
    this.authenticationMethod = this.requiresUsernamePasswordAuthentication
  ...
  protected sendConnectionResponse(
    addressType: number,
    ...
    const responseBytes: number[] = [version, connectionStatus, 0x00, addressType]
